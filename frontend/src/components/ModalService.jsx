import React from 'react';
import Modal from '@mui/material/Modal';
import Box from '@mui/material/Box';
import Button from '@mui/material/Button';
import ServPng1 from '../assets/services1.png';
import ServPng2 from '../assets/services2.png';
import ServPng3 from '../assets/services3.png';
import { useTranslation } from 'react-i18next';

const style = {
  position: 'absolute',
  top: '50%',
  left: '50%',
  transform: 'translate(-50%, -50%)',
  width: 400,
  bgcolor: 'background.paper',
  boxShadow: 24,
  textAlign: 'center',
  borderRadius: 8,
  pt: 2,
  px: 4,
  pb: 3,
};

export default function ModalService({ onClose }) {
  const handleClose = () => {
    onClose();
  };
  const { t } = useTranslation();

  return (
    <Modal
      open={true}
      onClose={onClose}
      aria-labelledby="child-modal-title"
      aria-describedby="child-modal-description"
    >
      <Box sx={{ ...style, width: 900 }}>
        <h2 id="parent-modal-title">{t('services.title')}</h2>
        <div className="services-divs">
          <div className="services-div">
            <img src={ServPng1} alt="" />
            <h2>{t('services.head1')}</h2>
            <p>{t('services.text1')}
            </p>
          </div>
          <div className="services-div">
            <img src={ServPng2} alt="" />
            <h2>{t('services.head2')}</h2>
            <p>{t('services.text2')}
            </p>
          </div>
          <div className="services-div">
            <img src={ServPng3} alt="" />
            <h2>{t('services.head3')}</h2>
            <p>{t('services.text3')}
            </p>
          </div>
        </div>

        <Button onClick={handleClose}>{t('services.close')}</Button>
      </Box>
    </Modal>
  );
}
